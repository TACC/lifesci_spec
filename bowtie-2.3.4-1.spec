...
make RELEASE_BUILD=1 CC=icc CXX=icpc AR=xiar WITH_AFFINITY=1 RELEASE_FLAGS="-O3 -m64 %{TACC_OPT} -ipo -I${TI}" LDFLAGS="-L${TL} -Wl,-rpath,${TL}" prefix=%{INSTALL_DIR} DESTDIR=${RPM_BUILD_ROOT} -j1
make RELEASE_BUILD=1 CC=icc CXX=icpc AR=xiar WITH_AFFINITY=1 RELEASE_FLAGS="-O3 -m64 %{TACC_OPT} -ipo -I${TI}" LDFLAGS="-L${TL} -Wl,-rpath,${TL}" prefix=%{INSTALL_DIR} DESTDIR=${RPM_BUILD_ROOT} -j1 install
