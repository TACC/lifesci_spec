...
%define shortsummary Parallel genome assemblies for parallel DNA sequencing
Summary: %{shortsummary}

# Give the package a base name
%define pkg_base_name ray
...
Source:    Ray-%{pkg_version}.tar.bz2
...
ID=$RPM_BUILD_ROOT/%{INSTALL_DIR}
make CXXFLAGS="-O3 %{TACC_OPT} -std=c++98 -ipo" PREFIX=$ID \
	HAVE_LIBZ=y HAVE_LIBBZ2=y CLOCK_GETTIME=y MPI_IO=y clean
make -j 4 CXXFLAGS="-O3 %{TACC_OPT} -std=c++98 -ipo" PREFIX=$ID \
	HAVE_LIBZ=y HAVE_LIBBZ2=y CLOCK_GETTIME=y MPI_IO=y
make CXXFLAGS="-O3 %{TACC_OPT} -std=c++98 -ipo" PREFIX=$ID \
	HAVE_LIBZ=y HAVE_LIBBZ2=y CLOCK_GETTIME=y MPI_IO=y install
...
whatis("Keywords: Biology, Genomics, Denovo, Assembler, Assembly, Metagenomics")
